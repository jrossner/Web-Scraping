...
def checkForUpcomingExDividend():
  ...
  if lastDayToOwn == (today+timedelta(days=1)).strftime('%Y-%m-%d'):
    ...
    text = f'Apple REIT\'s Ex-Dividend Date is 1 Week from today. Whoever owns APLE stock by the end of {(today+timedelta(days=6)).strftime('%A %B %d')} will receive the next dividend.'
  else:
    text = False
